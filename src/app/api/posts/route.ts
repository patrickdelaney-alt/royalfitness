...
export async function POST(req: NextRequest) {
  try {
    ...
    let post;
    try {
      post = await prisma.$transaction(async (tx: any) => {
      ...
      // The pre-check above closes the common case, but a concurrent request
      // (double-submit, retry) can still race past it and hit the unique
      // constraint here. Surface the same on-brand cooldown message instead
    ...
    await invalidateStreakCache(userId).catch(() => {});
