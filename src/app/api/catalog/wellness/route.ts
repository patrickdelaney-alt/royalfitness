...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const existing = await prisma.savedWellness.findUnique({ where: { id } });
    if (!existing) {
      return NextResponse.json({ error: "Wellness item not found" }, { status: 404 });
    }
    ...
    const body = await req.json();
    const data = catalogWellnessSchema.partial().parse(body);

    const updated = await prisma.savedWellness.update({
      where: { id },
      data: {
    ...
    console.error("PATCH /api/catalog/wellness error:", error);
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
